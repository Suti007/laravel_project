...
<body>
    ...
    return $item->post->price * $item->quantiy; })}}</strong></span>
                                    ...
                                    @if (session()->has('coupom'))
                                        <div class="d-flex justify-content-between mb-1 small">
                                            <span>Coupon(Code: NEWYEAR):({{session()->get('coupom')['name']}});
                                                <form action="{{route('coupom.destroy')}}" method="POST"
                                ...
                                <div class="mb-4">
                                    <label for="card-element">Credits or Debit Cards:</label>
                                    {{-- <div id="card-element">

                                    </div> --}}
                                    {{-- <form method="POST" action="{{route('checkout.payment', $newTotal)}}"
                                        id="stripe-form"> --}}
                                        @csrf
